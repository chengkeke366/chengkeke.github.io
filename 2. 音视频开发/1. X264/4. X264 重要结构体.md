...
int   i_pic_struct;     // 帧的结构类型，表示是帧还是场，是逐行还是隔行，取值为枚举值 pic_struct_e，定义在x264.h中
...
## x264_nal_t 编码输出的数据 x264_nal_t

x264_nal_t中的数据在下一次调用x264_encoder_encode之后就无效了，因此必须在调用x264_encoder_encode 或 x264_encoder_headers 之前使用或拷贝其中的数据。
...
int i_trellis;      // Trellis量化提高效率，对每个8x8的块寻找合适的量化值，需要CABAC，0 ：即关闭  1：只在最后编码时使用 2：在所有模式决策上启用
...
float f_rf_constant_max;    // 最大码率因子，该选项仅在使用CRF并开启VBV时有效， 图像质量的最大值，可能会导致VBV下溢。
...
/* MBTree File是一个临时文件，记录了每个P帧中每个MB被参考的情况。目前mbtree只处理P帧的MB，同时也不支持b_pyramid. */
